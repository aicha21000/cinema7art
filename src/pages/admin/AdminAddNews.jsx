...
function AdminAddNews() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setError(error.response?.data?.message || 'حدث خطأ في إضافة الخبر');
        } finally {
            setLoading(false);
        }
    };
                        ...
                        >
                            {newsService.getCategories().map(category => (
                                <option key={category} value={category}>
                                    {category === 'movie' && 'أفلام'}
                                    {category === 'actor' && 'ممثلون'}
                                    {category === 'award' && 'جوائز'}
                                    {category === 'industry' && 'صناعة السينما'}
                                </option>
                            ))}
                        </select>
                    </div>
